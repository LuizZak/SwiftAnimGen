/// Base class for producing Python-style code string buffers during code generation.
public class CodeStringBuffer {
    ...
    /// suffixes each line with the comment delimiter '#' and a line feed at the
    /// end.
    ...
    /// and suffixes each line with the comment delimiter '##' and a line feed
    /// at the end.
...
public extension CodeStringBuffer {
    ...
    /// Emits `lead`, then a colon, a newline, indents by one, invokes `block`
    /// and finally unindents before emitting a line feed:
    ///
    /// ```
    ...
    /// Emits a colon, a newline, indents by one, invokes `block` and
    /// finally unindent before emitting a line feed:
    ///
    /// ```
    ...
    /// Emits a colon, a newline, indents by one, invokes `block` and
    /// finally unindent before emitting newline, removing any empty line trailing
    /// the right brace:
    ///
    /// ```
    /// <current line's contents>:
    ...
    /// Emits a colon, and single line with the 'pass' keyword within:
    ///
    /// ```
    /// <current line's contents>:
    ///     pass
